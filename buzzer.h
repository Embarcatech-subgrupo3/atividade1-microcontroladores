...
char leitura_tecla() {
    ...
    if (gpio_get(Coluna4)) {  // Verifica se a tecla 'A' foi pressionada
        sleep_ms(50);  // Debounce
        while (gpio_get(Coluna4));  // Aguarda até a tecla ser solta
...
buzzer(buz_on){
  char tecla = leitura_tecla();
        if (tecla == 'A') {  // Se a tecla 'A' foi pressionada alterar o estado atual do buzzer
            if (buz_on) {
